...
txPulse = signal.hilbert(signal.chirp(np.linspace(0,1e-6,1e-6*rate,endpoint=False), f0=5e6, f1=20e6, t1=1e-6, phi=90, method='linear')).astype(np.complex64)
numTxSamps=len(txPulse)+header_len+footer_len
numRxSamps=1000
pulseScaleFactor = 1
...
rx_sdr = SoapySDR.Device(dict(driver="uhd", serial = serials[1]))
if not tx_sdr.hasHardwareTime():
    raise Exception('this device does not support timed streaming')
...
rxBw = 56e6
txBw = 56e6
if rxBw is not None: rx_sdr.setBandwidth(SOAPY_SDR_RX, rxChan, rxBw)
if txBw is not None: tx_sdr.setBandwidth(SOAPY_SDR_TX, txChan, txBw)
print('RX Bandwidth: {} MHz'.format(1e-6*rx_sdr.getBandwidth(SOAPY_SDR_RX, rxChan)))
print('TX Bandwidth: {} MHz'.format(1e-6*tx_sdr.getBandwidth(SOAPY_SDR_TX, txChan)))
...
def txRx(sampleOffset): 
    ...
    # rxBuffs = np.conjugate(rxBuffs)
    freqTx = np.fft.fftfreq(len(txSig), 1/rate)
    freq_index = np.argsort(freqTx)
    freqTx = freqTx[freq_index]
    fftTx = np.fft.fft(txSig)[freq_index]
    ...
    # fftAx[1].axis((-1e-6*rate/2, 1e-6*rate/2, 0, 10))
    fftAx[1].grid()

    sigMin = min([min(np.real(rxBuffs)), min(np.imag(rxBuffs))])
    sigMax = max([max(np.real(rxBuffs)), max(np.imag(rxBuffs))])
    vertPoints = np.linspace(sigMin, sigMax, 100)
    indsMin = [8, 215, 325]
    indsMax = [136, 265, numRxSamps]
    subFig, subAx = plt.subplots(len(indsMin)+1,1)
    subAx[0].plot(t,np.real(rxBuffs))
    subAx[0].plot(t,np.imag(rxBuffs))
    for ix in range(len(indsMin)):
        subSig = rxBuffs[indsMin[ix]:indsMax[ix]]
        subAx[0].plot(indsMin[ix]*np.ones(100), vertPoints, 'k--')
        subAx[0].plot(indsMax[ix]*np.ones(100), vertPoints, 'k--')
        subAx[0].text(indsMin[ix]+(indsMax[ix]-indsMin[ix])/2, 
            sigMax, str(ix+1), 
            horizontalalignment='center',
            verticalalignment='top')
        freq = np.fft.fftfreq(len(subSig), 1/rate)
        freq_index = np.argsort(freq)
        freq = freq[freq_index]
        fft = np.fft.fft(subSig)[freq_index]
        subAx[ix+1].plot(freq/1e6, np.abs(fft), 'b')
        subAx[ix+1].set_title(str(ix+1))
